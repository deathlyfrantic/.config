...
function set_up_prompt {
    ...
    local _path='%{$fg[white]%}${${(%):-%~}//\//"\
"%{$fg_bold[black]%\}/%{$reset_color%\}}'
    local _jobs="%(1j.%{$fg_bold[black]%} :: %{$reset_color%}"\
"jobs%{$fg_bold[black]%}/%{$fg_bold[blue]%}%j.)"
    ...
    local _base="$_path$_jobs %(!.%{$fg_bold[red]%}#.%{$_green%}$)"\
"%{$reset_color%} "
    ...
    RPROMPT="%{$reset_color%}%D{%H}%{$fg_bold[black]%}:"\
"%{$reset_color%}%D{%M}%{$fg_bold[black]%}:%{$reset_color%}%D{%S}"
